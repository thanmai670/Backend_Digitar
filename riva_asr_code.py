...
class ASRPipe(object):
    ...
    def __init__(self):
        ...
        self.sampling_rate = (
            asr_config["SAMPLING_RATE"]
            if "SAMPLING_RATE" in asr_config
            else SAMPLING_RATE
        )
        self.language_code = (
            asr_config["LANGUAGE_CODE"]
            if "LANGUAGE_CODE" in asr_config
            else LANGUAGE_CODE
        )
        self.enable_automatic_punctuation = (
            asr_config["ENABLE_AUTOMATIC_PUNCTUATION"]
            if "ENABLE_AUTOMATIC_PUNCTUATION" in asr_config
            else ENABLE_AUTOMATIC_PUNCTUATION
        )
        self.stream_interim_results = (
            asr_config["STREAM_INTERIM_RESULTS"]
            if "STREAM_INTERIM_RESULTS" in asr_config
            else STREAM_INTERIM_RESULTS
        )
    ...
    def start(self):
        if self.verbose:
            print(
                "[Riva ASR] Creating Stream ASR channel: {}".format(
